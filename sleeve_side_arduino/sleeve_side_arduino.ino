...
void loop() {  
  ...
  if(forward_processed<=5&&forward_processed>=-5) forward_processed=0;
  int side_raw=analogRead(A3);
  int side_processed=map(side_raw, 0, 1023, 0, 200);
  if(side_processed<=5&&side_processed>=-5) side_processed=0;
  int button_left_val=digitalRead(2);
  int button_right_val=digitalRead(3);
  sprintf(str, "%03d,%03d,%03d,%03d,%01d,%01d", side_processed,forward_processed,pan_processed,tilt_processed,button_left_val,button_right_val);
  Serial.println(str);
  delay(100);
}
